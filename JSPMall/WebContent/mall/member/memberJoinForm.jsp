...
<script>
	...
	document.addEventListener("DOMContentLoaded", function(){
		...
		// ID 중복 체크 버튼 - DB 작업 : member 테이블에 똑같은 id가 있는지 검사
		let btn_chk_id = document.getElementById("btn_chk_id");
		btn_chk_id.addEventListener("click", function(){
			isChk = true;
			if(form.id.value.length < 4){ // 아이디를 네글자 미만 입력했을때
				alert("아이디를 4글자 이상 입력해주세요.");
				form.id.focus();
			} else{ // 아이디를 네글자 이상 입력했을때
				location = 'memberIdCheck.jsp?id=' + form.id.value; //DB 작업을 위해 id를 함께 넘김
			}
		})
