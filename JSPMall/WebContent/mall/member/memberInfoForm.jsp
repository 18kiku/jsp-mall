...
#btn_address { width: 100px; height: 28px; border: none; background: #1e94be; color: white; font-size: 12px; 
cursor: pointer; border-radius: 3px; margin-left: 10px}
table span { font-size: 0.8em;}
/* 하단 - 가입, 취소 버튼*/
.btns { text-align: center; margin-top: 30px;}
.btns input[type="button"] { width: 200px; height: 40px; background: black; color: white; border: none;
	...
	document.addEventListener("DOMContentLoaded", function(){
		...
		form.pwd.addEventListener("keyup", function(){
			if(form.pwd.value.length < 4){
				chk_pwd.innerText = "비밀번호는 4글자 이상이어야합니다.";
		...
		<form action="memberUpdatePro.jsp" method="post" name="infoForm">
			<table>
				<tr>
					<th>아이디</th>
					<td>
						<input type="text" name="id" class="c_id" value="<%=member.getId()%>" readonly>
				...
				<tr>
					<th>이메일</th>
					<td>
						<input type="email" name="email" size=30 value="<%=member.getEmail()%>"><br>
						<span id="chk_email"></span>					
					</td>
				</tr>
				<tr>
					<th>전화번호</th>
					<td><input type="tel" name="tel" value="<%=member.getTel()%>"></td>
				</tr>
				<tr class="addr_row">
					<th>주소</th>
					<td>
						<input type="button" value="주소찾기" id="btn_address" value="<%=member.getAddress()%>"><br>
