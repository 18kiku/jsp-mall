...
.d2 span { font-weight: bold;}
.d_line { clear: both; width: 90%; border: 1px solid lightgray;}
/* 상단 - d3 */
.d3 { width: 90%; padding: 15px; margin: 0 45px; float: left;}
...
.d3 #btn_buy_select { background: #000; border: 1px solid #000;}
.d3 #btn_delete_select { background: #fff; border: 1px solid #000; color: #000;}
/* 중단 - 상품 정보 테이블 */
.cart_list { width: 100%;}
...
.t_cart_list th, .t_cart_list td { border: 1px solid #dee2e6; border-bottom: 1px solid #dee2e6; border-left: none; border-right: none; }
...
.td4 input[type="submit"] { width: 53px; height: 25px; border: 1px solid gray; background: #fff; border-radius: 2px; font-size: 0.9em; cursor: pointer;}
.td7 input[type="button"] { width: 80px; height: 30px; border-radius: 3px; font-weight: bold; cursor: pointer;}
.td7 .btn_buy_one { background: #000; border: 1px solid #000; margin-bottom: 5px; color: #fff;}
.td7 .btn_delete_one { background: #fff; border: 1px solid #000; color: #000;}

/* 하단 - d4 */
.d4 { width: 90%; padding: 15px; margin: 10px 45px;}
.d4 span { font-size: 0.9em; font-weight: bold; color: gray; margin-right: 10px; margin-left: 20px;}
.d4 input[type=button] { width: 80px; height: 30px; border-radius: 3px; color: #fff; font-weight: bold; cursor: pointer;}
.d4 #btn_buy_select2 { background: #000; border: 1px solid #000;}
.d4 #btn_delete_select2 { background: #fff; border: 1px solid #000; color: #000;}

/* 하단 - 총상품금액 테이블 */
.t_cart_tot { width: 90%; border: 1px solid dee2e6; border-collapse: collapse; margin: 0 auto; font-size: 0.9em; 
clear: both; background: #f1f7f9;}
.t_cart_tot tr { height: 100px;}
.t_cart_tot th, .t_cart_tot td { }
.t_cart_tot tr th:last-of-type { background: #f1f3f5;}
...
.t_cart_tot tr:nth-child(2) { font-size: 0.8em; text-align: center;}
.t_cart_tot tr:nth-child(2) span { font-weight: normal;}
.t_cart_tot tr:nth-child(2) span b { color: #a13b66;}
.t_cart_tot tr:nth-child(2) th { border-top: 1px solid #dee2e6;}

/* 하단 - 배송지 테이블 */
.t_cart_address { width: 90%; border: 1px solid gray; border-collapse: collapse; margin: 20px auto; font-size: 0.9em; 
border-left: none; border-right: none; clear: both;}
.t_cart_address tr { height: 60px;}
.t_cart_address td, .t_cart_address th { border: 1px solid #dee2e6; border-bottom: 1px solid #dee2e6; border-left: none; border-right: none; }
...
#btn_address { cursor: pointer;}

/* 하단 - 주문, 쇼핑계속하기 버튼 */
.d5 { width: 90%; padding: 15px; margin: 0 45px 30px; text-align: center;}
.d5 input[type=button] { width: 150px; height: 50px; font-size: 1.05em; font-weight: bold; cursor: pointer; border-radius: 3px;}
.d5 #btn_buy_select3 { background: #000; border: 1px solid #000; color: #fff;}
.d5 #btn_shopping { background: #fff; border: 1px solid #000; color: #000;}
</style>
...
<script>
	document.addEventListener("DOMContentLoaded", function(){
		...
		ck_cart_all.addEventListener("change", function(){
			...
			k_count = 0;
			p_count = 0;
			if(ck_cart_all.checked == true){ // 전체선택을 체크했을때 -> 각 상품의 모든 체크박스 선택
			...
			} else {						 // 전체 선택을 해제했을때 -> 각 상품의 모든 체크박스를 해제
				ck_count = 0;
				cart_ids_list = [];
				for(let i=0; i<ck_cart_ones.length; i++){
			...
			c1_s2.innerHTML = p_s1.toLocaleString() + '원';
		...
		btn_delete_select.addEventListener("click", function(){
			if(ck_count == 0) { alert('장바구니에 상품이 없습니다.'); return;}
			location = "cartDeletePro.jsp";
		})
...
<body>
	<div class="container">
		...
		<div class="cart_list">
			...
			<div class="d2">
				<a href="/JSPMall/index.jsp">홈</a> > <span>장바구니</span>
			</div>
			<hr class="d_line">
			...
			<table class="t_cart_list">
				...
				<%for(CartDTO cart : cartList) {
					...
					p_tot += p_sum;					// 총 합계 (할인가격의 총합계)
					
					p_sum2 = product_price * buy_count; // 각 상품의 합계 (원가)
					p_tot2 += p_sum2;					// 총합계(원가)
					
					p_sum3 = product_price * discount_rate / 100; // 각 상품의 할인된 가격, ex)1000 * 10 / 100
					p_tot3 += p_sum3 * buy_count;				  // 각 상품의 할인된 가격의 총합계
				...
				<input type="hidden" name="p_sum3" value="<%=p_sum3%>">
					<tr>
						<td class="center td1" width="3%"><span><input type="checkbox" name="ck_cart_one" class="ck_cart_one"></span></td>
						<td class="center td2" width="8%">
						<a href="../shopping/shopContent.jsp?product_id=<%=cart.getProduct_id()%>"><img src="/images_jspmall/<%=cart.getProduct_image()%>" width="60" height="60"></a></td>
						<td class="left td3" width="48%">
							...
							<span class="s2"><%=product.getProduct_size() %> | <%=product.getProduct_color() %></span><br>
						...
						<td class="center td4" width="8%">
							<input type="number" name="buy_count" value="<%=buy_count %>" class="buy_count" min=1 max=99><br>
						...
						<td class="center td7" width="12%">
							<input type="button" name="btn_buy_one" value="주문하기" class="btn_buy_one"><br>
				...
				<%} }%>
				<tr>
					<th colspan="7">배송 상품 총 금액 : <b class="c1_s1">0원</b></th>
				</tr>
			</table>
			<div class="d4">
				<span>선택한 상품</span>
			...
			<table class="t_cart_address">
				<tr>
					...
					<td>배송지 : <span id="address"><%=address %></span> <input type="button" value="배송지 변경" id="btn_address"></td>
				</tr>
			</table>
			<div class="d5">
				<input type="button" value="주문하기" id="btn_buy_select3">
